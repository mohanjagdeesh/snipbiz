...
const Messages = () => {
  ...
  const messageDesplay = () => (
    <tr className="trow" data-testid="message-details">
      <td colSpan="3">
  ...
  return (
    <div className="Messages-Profile" data-testid="messages-profile">
      <div className="nav">
    <table className="table1" cellPadding="15" data-testid="message-table">
      <caption className="caption">Message </caption>
      <thead>
        <tr className="trow">
          <th className="tablehead" width="70%" data-testid="table-header-subject">Subject</th>
          <th className="tablehead"data-testid="table-header-send-date">Send Date</th>
          <th className="tablehead"data-testid="table-header-delete">Delete</th>
        </tr>
      </thead>
      <tbody className="tbody">
        {tableData.map((eachData) => (
          <>
            <tr className="trow" key={eachData.id} onClick={() => onClickTableData(eachData.id)} data-testid={`table-row-${eachData.id}`}>
              <td className="tdata" data-testid={`table-row-subject-${eachData.id}`}>{eachData.subject}</td>
              <td className="tdata" data-testid={`table-row-send-date-${eachData.id}`}>{eachData.sendDate}</td>
              <td className="tdata" data-testid={`table-row-delete-${eachData.id}`}><MdDeleteOutline/></td>
            </tr>
